<template lang="pug">
  div
    ...
    .activity1-other.fill-view.bg-gray(v-show="!nestedView")
      ...
      .cell-wrapper
        ...
        title="使用时段"
        value="设置"
        v-on:click.native="$router.push({name: 'activity1.useTime'})"
        )
        mt-cell(
        is-link
        title="可以转赠"
      ...
      .cell-wrapper
        ...
        title="使用说明"
        value="设置"
        v-on:click.native="$router.push({name: 'activity1.useInstructions'})"
        )
...
export default {
  ...
  computed: {
    ...
    nestedView () {
      return this.$route.name !== 'activity1.other'
    }
  },
  components: {
    FormCell
  ...
  created () {
    let data = JSON.parse(JSON.stringify(this.$store.state.activity1Other.other))
    delete data.useTime
