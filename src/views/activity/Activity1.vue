<template lang="pug">
  div
    ...
    .page-activity1(v-show="!nestedView")
      top-head
      .page-title  消费满减
      .cell-wrapper
        form-cell(
        ...
        title="适用门店"
        value="请选择"
        v-on:click.native="$router.push({name: 'activity1.stores'})"
        )
      .cell-wrapper
        mt-cell(
        is-link
      ...
      .cell-wrapper
        ...
        v-on:click.native="$router.push({name: 'activity1.coupouExpired'})"
        )
      div.btn-list
        button.btn.btn-reverse.btn-other(v-on:click="$router.push({name: 'activity1.other'})")
          span  其他设置
...
import { activity1Validation } from './validation'
...
export default {
  ...
  computed: {
        ...
        promoTools: [
          {
            voucher: {
              ...
              maxAmount: this.highestDiscountLimit,
    ...
    nestedView () {
      return this.$route.name !== 'activity1'
    },
    startDate () {
  ...
  methods: {
    submit () {
      ...
      let formData = {
        ...
        useInstructions: this.other.useInstructions,
        highestDiscountLimit: this.highestDiscountLimit
      }
      console.log(`result: ${validateForm(formData, activity1Validation)}`)
      if (validateForm(formData, activity1Validation)) return true
