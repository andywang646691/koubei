<template lang="pug">
  div
    ...
    .discount2-other.fill-view.bg-gray(v-show="!nestedView")
...
export default {
  ...
  computed: {
    ...
    nestedView () {
      return this.$route.name !== 'discount2.other'
    }
  },
  components: {
    FormCell
  ...
  methods: {
    ...
    confirm () {
      ...
      this.setDiscount2Other(data)
      this.$router.push({name: 'discount2'})
    }
  },
  created () {
    let data = JSON.parse(JSON.stringify(this.$store.state.discount2Other.other))
    delete data.useTime
